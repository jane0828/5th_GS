                                 ...
                                 uint8_t protocol_version)
{
    if (protocol_version > 0) {
        ...
        while (remaining_samples > 0) {
            ...
            if (samples_to_request > GS_HK_REQUEST_BURST_BEACON_COUNT) {
                samples_to_request = GS_HK_REQUEST_BURST_BEACON_COUNT;
            }
            ...
            /* wait GS_HK_REQUEST_BURST_IDLE_MSECS to avoid flooding a slow sender
               in the server end - e.g. the ax100 uhf radio */
            gs_time_sleep_ms(GS_HK_REQUEST_BURST_IDLE_MSECS);
        }
    } else {
