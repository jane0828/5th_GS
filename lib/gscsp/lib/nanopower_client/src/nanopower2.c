...
void eps_hk_print(eps_hk_t * hk) {
	...
	default:printf("BM:  %3u", hk->battmode); break;
	}
	printf("         |  7         --> EN:%u\r\n", hk->output[7]);
	printf("              +-------------------+\r\n");
	printf("\r\n");
...
void eps_config3_unpack(eps_config3_t * pconf3)
{
	for (int i = 0; i < 8; i++) {
		pconf3->cur_lim[i] = csp_ntoh16(pconf3->cur_lim[i]);
	}
}
...
void eps_config3_pack(eps_config3_t * pconf3)
{
	for (int i = 0; i < 8; i++) {
		pconf3->cur_lim[i] = csp_hton16(pconf3->cur_lim[i]);
	}
}
...
void eps_config3_print(eps_config3_t * pconf3)
{
	for (int i = 0; i < 8; i++)
		printf("Current limit[%u]   %"PRIu16" mA\r\n", i, pconf3->cur_lim[i]);
	printf("Current EMA gain   %u\r\n", pconf3->cur_ema_gain);
	printf("Wdt CSP Channel    %u %u\r\n", pconf3->cspwdt_channel[0], pconf3->cspwdt_channel[1]);
	printf("Wdt CSP Address    %u %u\r\n", pconf3->cspwdt_address[0], pconf3->cspwdt_address[1]);
}
...
int eps_config_set(eps_config_t * pconfig)
{
	eps_config_t config;

	memcpy(&config, pconfig, sizeof(eps_config_t));
...
{
	int status = csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG2_GET, eps_timeout, NULL, 0, config2, sizeof(eps_config2_t));
	if (status > 0) {
...
int eps_config2_set(eps_config2_t * pconfig2)
{
	eps_config2_t config2;

	memcpy(&config2, pconfig2, sizeof(eps_config2_t));
	eps_config2_pack(&config2);
	return csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG2_SET, eps_timeout, &config2, sizeof(eps_config2_t), NULL, 0);
}

int eps_config2_cmd(uint8_t cmd)
{
	return csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG2_CMD, eps_timeout, &cmd, 1, NULL, 0);
}

int eps_config3_get(eps_config3_t * pconfig3)
{
	pconfig3->version = 1;
	pconfig3->cmd = EPS_CONFIG_CMD_GET;
	pconfig3->length = sizeof(eps_config3_t);
	pconfig3->flags = 0;
	int status = csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG3, eps_timeout, pconfig3, 4, pconfig3, sizeof(eps_config3_t));
	if (status > 0) {
		eps_config3_unpack(pconfig3);
	} else {
		status = -1;
	}
	return status;
...
int eps_config3_set(eps_config3_t * pconfig3)
{
	eps_config3_t config3;

	memcpy(&config3, pconfig3, sizeof(eps_config3_t));
	config3.version = 1;
	config3.cmd = EPS_CONFIG_CMD_SET;
	config3.length = sizeof(eps_config3_t);
	config3.flags = 0;
	eps_config3_pack(&config3);
	return csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG3, eps_timeout, &config3, sizeof(eps_config3_t), NULL, 0);
}

int eps_config3_cmd(uint8_t cmd)
{
	eps_config3_t config3;
	...
	config3.length = 4;
	config3.flags = 0;
	return csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_CONFIG3, eps_timeout, &config3, 4, NULL, 0);
}
...
int eps_heater(uint8_t heater, uint8_t mode, uint8_t *state)
{
	...
	data[1] = heater;
	data[2] = mode;
	int status = csp_transaction(CSP_PRIO_HIGH, eps_node, EPS_PORT_HEATER, eps_timeout, data, 3, state, 2);
			printf("%u %u\r\n",state[0],state[1]);
	return status;
}
