...
/*
  Defined here to remove compiler warning, because this function is no longer supported in libmbed - use gs_i2c_master_transaction()
  However, this module doesn't seem to be used anymore, therefor the code has not been updated to use newer APIs, until it also can be tested.
  Jira: NP31-29
*/
extern int i2c_receive(uint8_t handle, i2c_frame_t ** frame, uint16_t timeout);
...
int eps_slave_hk2_vi(struct command_context *ctx) {
	...
	frame->data[1] = 1;
	frame->len = 2;
	frame->len_rx = 2 + (uint8_t) sizeof(eps_hk_vi_t);
	frame->retries = 0;
...
int eps_slave_hk2_out(struct command_context *ctx) {
	...
	frame->data[1] = 2;
	frame->len = 2;
	frame->len_rx = 2 + (uint8_t) sizeof(eps_hk_out_t);
	frame->retries = 0;
...
int eps_slave_hk2_wdt(struct command_context *ctx) {
	...
	frame->data[1] = 3;
	frame->len = 2;
	frame->len_rx = 2 + (uint8_t) sizeof(eps_hk_wdt_t);
	frame->retries = 0;
...
int eps_slave_hk2_basic(struct command_context *ctx) {
	...
	frame->data[1] = 4;
	frame->len = 2;
	frame->len_rx = 2 + (uint8_t) sizeof(eps_hk_basic_t);
	frame->retries = 0;
	...
	hk = (eps_hk_basic_t *)&frame->data[2];
...
int cmd_eps_slave_config_confirm(struct command_context *ctx) {
...
int cmd_eps_slave_config2_get(struct command_context *ctx) {
	printf("Requesting EPS Config2\r\n");

	i2c_frame_t * frame;
	frame = csp_buffer_get(I2C_MTU);
	frame->dest = slave_node;
	frame->data[0] = EPS_PORT_CONFIG2_GET;
	frame->len = 1;
	frame->len_rx = 2 + (uint8_t) sizeof(eps_config2_t);
	frame->retries = 0;
	...
	memcpy(&nanopower_config2, &frame->data[2], sizeof(eps_config2_t));
...
int cmd_eps_slave_config2_print(struct command_context *ctx) {
	eps_config2_print(&nanopower_config2);
	return CMD_ERROR_NONE;
}

int cmd_eps_slave_config2_set(struct command_context *ctx) {
	printf("Setting EPS Config2\r\n");

	i2c_frame_t * frame;
	frame = csp_buffer_get(I2C_MTU);
	frame->dest = slave_node;
	frame->data[0] = EPS_PORT_CONFIG2_SET;
	frame->len = 1 + (uint8_t) sizeof(eps_config2_t);
	frame->len_rx = 2;
	frame->retries = 0;

	memcpy(&frame->data[1], &nanopower_config2, sizeof(eps_config2_t));
	eps_config2_pack((eps_config2_t *) &frame->data[1]);
	if (i2c_send(0, frame, 0) != E_NO_ERR) {
		csp_buffer_free(frame);
		return CMD_ERROR_NOMEM;
	}
...
int cmd_eps_slave_config2_restore(struct command_context *ctx) {


	i2c_frame_t * frame;
	frame = csp_buffer_get(I2C_MTU);
	frame->dest = slave_node;
	frame->data[0] = EPS_PORT_CONFIG2_CMD;
	frame->data[1] = 1;
	frame->len = 2;
	frame->len_rx = 0;
	frame->retries = 0;
...
int cmd_eps_slave_config2_confirm(struct command_context *ctx) {
...
int cmd_eps_slave_config2_edit(struct command_context *ctx) {
...
command_t __sub_command eps_slave_config2_subcommands[] = {
	{
		.name = "get",
		.help = "Conf2 get",
...
command_t __sub_command eps_slave_hk_subcommands[] = {
	{
		.name = "vi",
...
command_t __sub_command eps_slavecommands[] = {
	{
		...
		.chain = INIT_CHAIN(eps_slave_hk_subcommands),
	},{
		.name = "outputmask",
		...
		.name = "conf2",
		.help = "Configuration2",
		.chain = INIT_CHAIN(eps_slave_config2_subcommands),
	}
};
...
command_t __root_command epsslave_commands[] = {
	{
		.name = "epsslave",
		.help = "client: NanoPower P31u (slave)",
