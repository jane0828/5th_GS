...
int gatoss_cmd_autoload_fpga(struct command_context * ctx) {

	if (ctx->argc != 2)
		return CMD_ERROR_SYNTAX;

	int8_t autoload = atoi(ctx->argv[1]);
	if (autoload == 1) {
		printf("Enabling fpga autoload\r\n");
	} else if (autoload == 0) {
		printf("Disabling fpga autoload\r\n");
	} else {
		return CMD_ERROR_SYNTAX;
	}

	int result = gatoss_autoload_fpga(autoload);
...
int gatoss_cmd_list_short(struct command_context *ctx) {
	...
	uint32_t packet_limit = atoi(ctx->argv[2]);
	uint32_t start_index = 0;

	if (ctx->argc == 4)
		start_index = atoi(ctx->argv[3]);

	if (gatoss_list_short(filtering, packet_limit, start_index) < 0)
		return CMD_ERROR_FAIL;
...
int gatoss_cmd_list_full(struct command_context *ctx) {
	...
	if (gatoss_list_full(filtering, packet_limit, start_index) < 0)
		return CMD_ERROR_FAIL;
...
int gatoss_cmd_plane_full(struct command_context *ctx) {
	...
	if (plane.icao24 == 0) {
		gs_color_printf(GS_COLOR_RED, "No such plane %" PRIx32 "\r\n", icao24);
		return CMD_ERROR_FAIL;
	}

	gs_color_printf(GS_COLOR_GREEN, "[%06"PRIX32"] ", plane.icao24);
	gs_color_printf(GS_COLOR_GREEN, "<%8s>", plane.id);
	gs_color_printf(GS_COLOR_GREEN, " C: %4"PRIu16"/%4"PRIu16" [%4"PRIu32"-%4"PRIu32"]", plane.position_counter, plane.frame_counter, plane.first_contact, plane.last_contact);
	gs_color_printf(GS_COLOR_GREEN, " La: %7.03f", pos.lat);
	gs_color_printf(GS_COLOR_GREEN, " Lo: %7.03f", pos.lon);
	gs_color_printf(GS_COLOR_GREEN, " A: %5"PRIu32, pos.altitude);
	gs_color_printf(GS_COLOR_GREEN, " H: %3.00f", plane.heading / 150.0);
	gs_color_printf(GS_COLOR_GREEN, " S: %03"PRIu16, plane.speed);
	gs_color_printf(GS_COLOR_GREEN, "\r\n");

	int i;
	printf("Positions received: %d\r\n", count);
	for (i = 0; i < count; i++) {
		gatoss_positions_decompress(&plane.positions[i], &pos);
		gs_color_printf(GS_COLOR_BLUE, "\t[%5"PRIu32"], La: %7.03f, Lo: %7.03f, A: %5"PRIu32"\r\n", pos.timestamp, pos.lat, pos.lon, pos.altitude);
	}
...
int gatoss_cmd_conf_current_limit(struct command_context *ctx) {
	...
	uint8_t curr_max_3v3_sd = atoi(ctx->argv[1]);
	uint8_t curr_max_1v2 = atoi(ctx->argv[2]);
	uint8_t curr_max_2v5 = atoi(ctx->argv[3]);
	uint8_t curr_max_3v3_fpga = atoi(ctx->argv[4]);
	uint8_t curr_max_3v3_adc = atoi(ctx->argv[5]);
	uint16_t curr_max_3v3_board = atoi(ctx->argv[6]);
	uint16_t curr_max_5v0_board = atoi(ctx->argv[7]);

	gatoss_conf.curr_max_3v3_sd = curr_max_3v3_sd;
	gatoss_conf.curr_max_1v2 = curr_max_1v2;
	gatoss_conf.curr_max_2v5 = curr_max_2v5;
	gatoss_conf.curr_max_3v3_fpga = curr_max_3v3_fpga;
	gatoss_conf.curr_max_3v3_adc = curr_max_3v3_adc;
	gatoss_conf.curr_max_3v3_board = curr_max_3v3_board;
	gatoss_conf.curr_max_5v0_board = curr_max_5v0_board;
...
int gatoss_cmd_conf_clear_latchup_channel(struct command_context *ctx) {
...
int gatoss_cmd_conf_other(struct command_context *ctx) {
	...
	gatoss_conf.eps_csp_addr = atoi(ctx->argv[2]);
	gatoss_conf.eps_adsb_pwr_chan = atoi(ctx->argv[3]);
	gatoss_conf_print(&gatoss_conf);
...
struct command gatoss_subcommands[] = {
	{
		...
		.name = "autoload",
		.help = "Autoload FPGA on boot",
		.usage = "<autoload>",
		.handler = gatoss_cmd_autoload_fpga,
	}, {
		.name = "load_fpga",
		...
		.name = "list_short",
		.help = "LIST_SHORT",
		.usage = "<filtering> <packet_limit> [index]",
...
#endif
	{
		...
		.name = "conf_curr_limit",
		.help = "Set current limits",
		.usage = "<3v3_sd> <1v2> <2v5> <3v3_fpga> <3v3_adc> <3v3_board> 5v0_board>",
		.handler = gatoss_cmd_conf_current_limit,
	}, {
		.name = "conf_other",
		.help = "Set other options",
		.usage = "<sd_enable> <eps_node> <adsb_pwr_chan>",
		.handler = gatoss_cmd_conf_other,
	}, {
		.name = "conf_clear_latchup",
		.help = "Clear over over current bits",
		.usage = "",
		.handler = gatoss_cmd_conf_clear_latchup_channel,
	}, {
		.name = "conf_print",
...
struct command __root_command gatoss_commands[] = {
	{
		.name = "gatoss",
		.help = "client: NanoCom ADS-B Receiver",
