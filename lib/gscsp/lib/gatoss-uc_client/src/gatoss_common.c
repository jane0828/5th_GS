...
void gatoss_position_compress(gatoss_pos_t * in, gatoss_compos_t * out) {
	...
	uint32_t lon_tmp = ((uint32_t) ((in->lon + 180.0) * 23300.0 + 0.5)) & 0x7FFFFF;
	uint32_t lat_tmp = ((uint32_t) ((in->lat + 90.0) * 23300.0 + 0.5)) & 0x3FFFFF;
	uint32_t alt_tmp = ((uint32_t) (in->altitude / 25 + 0.5)) & 0x7FF;
