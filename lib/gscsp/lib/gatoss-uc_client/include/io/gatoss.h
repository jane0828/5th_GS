...
#include <stdint.h>
#include <stdbool.h>
#include <conf_adsb_client.h>

#define GATOSS_NODE							4

#define GATOSS_MTU							202
...
typedef struct __attribute__((__packed__)) {
	...
	uint8_t curr_max_3v3_sd;							//! Maximum SD current allowed in mA
	uint8_t curr_max_1v2;								//! Maximum FPGA 1v2 current allowed in mA
	uint8_t curr_max_2v5;								//! Maximum FPGA 2v5 current allowed in mA
	uint8_t curr_max_3v3_fpga;							//! Maximum FPGA 3v3 current allowed in mA
	uint8_t curr_max_3v3_adc;							//! Maximum ADC 3v3 current allowed in mA
	uint16_t curr_max_3v3_board;						//! Maximum board 3v3 current allowed in mA
	uint16_t curr_max_5v0_board;						//! Maximum board 5v0 current allowed in mA
	uint8_t latchup_channel;							//! Bit mask that is set if over current if detected on a channel
	int8_t eps_csp_addr;								//! CSP addr for EPS. Used to power cycle ADS-B receiver if a latch-up is detected
	uint8_t eps_adsb_pwr_chan;							//! ADS-B payload power channel number
} gatoss_conf_t;
...
typedef struct __attribute__((__packed__)) gatoss_hk {
	...
	uint16_t current_3v3_sd;						//! This is sampled by HK task
...
struct __attribute__((__packed__)) gatoss_load_req {
	int8_t autoload;						//! Set to zero to disable auto load or 1 to enable autoload. Set to -1 to ignore this parameter
...
struct __attribute__((__packed__)) gatoss_list_req {
	...
	uint32_t packet_limit;							//! maximum number packets generated. 0 for unlimited
	uint32_t start_index;							//! Specify a start offset in the plane list
};
...
struct __attribute__((__packed__)) gatoss_plane_full {
	...
	gatoss_compos_t positions[(GATOSS_MTU - 40) / 11];					//! An array of positions
};
...
struct __attribute__((__packed__)) gatoss_plane_positions {
	...
	uint8_t  end_flag;								//! This is the last packet;
	gatoss_compos_t positions[(GATOSS_MTU - 6) / 11];					//! An array of positions
};
...
/**
 * Enable or disable fpga auto load on boot
 * @param autoload 0 disable autoload 1 to enable autoload
 * @return 0 = ok
 */
 ...
 * @param filtering 0 for no filtering and 1 for filtering. Filtering means that only planes with a position lock are returned
 * @param packet_limit limits number of packets return to this number. 0 == unlimited
 * @param start_index used as a start index in the plane list.
 * @return -1 if err
 */
