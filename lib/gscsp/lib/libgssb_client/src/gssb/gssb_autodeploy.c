...
#define ANT6_START_BURNTIME         7
#define ANT6_INCREMENT              3
#define AR6_START_BURNTIME          1
                                  ...
                                  uint8_t channel_board_2, uint32_t seconds_since_boot, uint8_t *current_state)
{
    ...
    /* If not released, tries less than max and not burning - initiate burn */
    ...
    /* If double burntime passed and not released - switch flag, and make sure that we are not burning */
    else if ((seconds_since_boot >= (burn_start_time + burntime * 2)) && (burning == 1)
                    && ((release_status_board_1 != 1) || (release_status_board_2 != 1))) {
        burning = 0;
    ...
    /* If either released or max retries used - stop burn and switch state */
    else if (((release_status_board_1 == 1) && (release_status_board_2 == 1))
                    || (tries_1 >= MAX_RETRIES_SINGLE_CHANNEL)) {
        burning = 0;
        tries_1 = 0;
        tries_2 = 0;
        (*current_state)++;
...
static int8_t ar6_burn_algorithm(uint8_t i2c_addr, uint32_t seconds_since_boot, uint8_t *current_state)
{
    ...
    /* If not released, tries less than max and not burning - initiate burn */
    if ((status.status != 1) && (tries < MAX_RETRIES_SINGLE_CHANNEL) && (burning == 0)) {
        burning = 1;
    ...
    /* If double burntime passed and not released - switch flag, and make sure that we are not burning */
    else if ((seconds_since_boot >= (burn_start_time + burntime * 2)) && (burning == 1) && (status.status != 1)) {
        burning = 0;
    ...
    /* If either released or max retries used - stop burn and switch state */
    else if ((status.status == 1) || (tries >= MAX_RETRIES_SINGLE_CHANNEL)) {
        burning = 0;
        tries = 0;
        (*current_state)++;
        res = gs_gssb_common_stop_burn(i2c_addr, i2c_timeout_ms);
                                         ...
                                         uint8_t max_burn_time)
{
    ...
    for (uint8_t i = 0; i <= 1; i++) {
        ...
        do {
            ...
            tries++;
            gs_time_sleep_ms(burn_time * 1500);
