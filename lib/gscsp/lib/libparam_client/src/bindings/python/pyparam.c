...
static PyObject* pyrparam_set_string(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("i", rparam_set_string(in, inlen, addr, index_id, node, port, timeout));
}
...
static PyObject* pyrparam_set_data(PyObject *self, PyObject *args)
{
    ...
    if (error != GS_OK) {
        ...
        return Py_BuildValue("i", rparam_set_string(outbuffer, inlen, addr, index_id, node, port, timeout));
    }
}
