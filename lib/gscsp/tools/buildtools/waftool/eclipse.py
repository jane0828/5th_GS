...
class eclipse(Build.BuildContext):
	...
	def create_cproject(self, appname, workspace_includes=[], pythonpath=[], symbols=[]):
		"""
		...
		for g in self.groups:
			for tg in g:
				...
				symbolnodes = tg.to_list(getattr(tg, 'defines', []) + tg.env['DEFINES'])
                                for s in symbolnodes:
                                        if s not in symbols:
                                                symbols.append(s)
                                                print "symbol: " + str(s)
                ...
                print "found " + str(len(workspace_includes)) + " include paths"
                print "found " + str(len(symbols)) + " symbols"

		project = self.impl_create_project(sys.executable, appname)
		self.srcnode.make_node('.project').write(project.toxml())

		waf = os.path.abspath(sys.argv[0])
		project = self.impl_create_cproject(sys.executable, waf, appname, workspace_includes, cpppath, source_dirs, symbols)
		self.srcnode.make_node('.cproject').write(project.toxml())

		if not os.path.exists('.settings'):
			os.makedirs('.settings')

		settings = self.impl_create_settings(sys.executable, waf, appname, workspace_includes, cpppath, source_dirs, symbols)
	...
	def impl_create_cproject(self, executable, waf, appname, workspace_includes, cpppath, source_dirs=[], symbols=[]):
		doc = Document()
	...
	def impl_create_settings(self, executable, waf, appname, workspace_includes, cpppath, source_dirs=[], symbols=[]):
