...
main() {
    validate_config
    log_git_start
    append_state "RUNNING" "resume from Phase 4 Integration"
    log "Runner resume start mode=phase4 run_id=$RUN_ID"

    if [[ "$ALLOW_AUTONOMOUS_SOURCE_CHANGES" != "1" ]]; then
        fail_runner "Phase 4 requires ALLOW_AUTONOMOUS_SOURCE_CHANGES=1"
    fi

    run_orchestrator "Phase 4 dispatch" "Resume from Phase 4. Phase 1-3 were already completed and approved in the previous run. Dispatch Integration Worker only."
    ...
    CURRENT_AGENT="none"
    append_state "FINAL_REPORTED" "phase4 resume workflow complete"
    finalize_git_state
