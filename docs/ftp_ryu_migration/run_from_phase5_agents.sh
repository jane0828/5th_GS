...
main() {
    validate_config
    log_git_start
    append_state "RUNNING" "resume from Phase 5 Hardening/Diagnostics"
    log "Runner resume start mode=phase5 run_id=$RUN_ID"

    run_orchestrator "Phase 5 dispatch" "Phase 4 passed after targeted re-review. Dispatch Runtime update and Parser Hardening Workers."
    run_parallel_workers "Phase 5 Hardening/Diagnostics" \
        "Perform Phase 5 execution/update only. Distinguish optional and applied patches and preserve rollback. Phase 4 targeted re-review returned PHASE_GO_WITH_NOTES after Agent 04b resolved Ryu pthread argument ownership." \
        agent_06_runtime_connection_diag agent_07_parser_hardening
    run_phase_review "Phase 5 Hardening/Diagnostics" \
        agent_06_runtime_connection_diag agent_07_parser_hardening

    run_orchestrator "Phase 6 dispatch" "Phase 5 passed. Prepare the final independent Supervisor audit."
    ...
    CURRENT_AGENT="none"
    append_state "FINAL_REPORTED" "phase5 resume workflow complete"
    finalize_git_state
