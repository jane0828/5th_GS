...
main() {
    validate_config
    log_git_start
    append_state "RUNNING" "resume from Phase 6 Supervisor/Final Manager"
    log "Runner resume start mode=supervisor_final run_id=$RUN_ID"

    run_orchestrator "Phase 6 dispatch" "Phase 1-5 passed with notes. Prepare final independent Supervisor audit. Carry open risk: live Ryu/OBC runtime validation remains pending."
    if run_supervisor_cycle; then
        run_final_manager "Normal final report mode. Preserve the Supervisor decision exactly. Carry open risk that live Ryu/OBC runtime validation remains pending."
    else
    ...
    CURRENT_AGENT="none"
    append_state "FINAL_REPORTED" "supervisor/final workflow complete"
    finalize_git_state
